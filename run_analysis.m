...
function run_analysis()
    ...
    pool = gcp('nocreate');
    if isempty(pool)
        parpool('local', max_workers);  % uses default number of workers
    end
    ...
    % ITERATING THROUGH IMAGES        
        ...
        % For each image size, perform the following:
        %   a. Measure execution time of mandelbrot_serial
    
    
        % ---- Serial timing ----
        for i = 1:5% Iterating multiple runs to find colosest to theoretical max
        ...
        time_parallel(r) = min(time_parallel_for_loop);
        ...
        %   methods
        % Temp commented out for faster profiling
